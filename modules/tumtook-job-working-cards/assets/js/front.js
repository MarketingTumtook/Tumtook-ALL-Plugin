(function () {
  var config = {
    root: "[data-ttwc-slider]",
    track: "[data-ttwc-track]",
    slide: ".ttwc-card",
    dotClass: "ttwc-dot",
    dots: "[data-ttwc-pagination]",
    prev: "[data-ttwc-prev]",
    next: "[data-ttwc-next]",
    cssPrefix: "ttwc",
    pageLabel: "Go to job group "
  };
  ...
  function setupSlider(root) {
    ...
    track.addEventListener("click", function (event) {
      ...
      card = event.target.closest(".ttwc-card[data-card-url]");
  ...
  function getDynamicRefreshUrl(root) {
    var ajaxUrl = root.getAttribute("data-ttwc-ajax-url") || "";
    var action = root.getAttribute("data-ttwc-action") || "ttwc_refresh_items";
    var restUrl = root.getAttribute("data-ttwc-rest-url") || "";
    var postId = root.getAttribute("data-ttwc-post-id") || "0";
    var limit = root.getAttribute("data-ttwc-limit") || "0";
    var optionalParams = {
      post_type: root.getAttribute("data-ttwc-post-type") || "",
      taxonomy: root.getAttribute("data-ttwc-taxonomy") || "",
      category: root.getAttribute("data-ttwc-category") || "",
      category_id: root.getAttribute("data-ttwc-category-id") || "",
      info_meta: root.getAttribute("data-ttwc-info-meta") || "",
      badge_meta: root.getAttribute("data-ttwc-badge-meta") || "",
      image_meta: root.getAttribute("data-ttwc-image-meta") || "",
      orderby: root.getAttribute("data-ttwc-orderby") || "",
      include: root.getAttribute("data-ttwc-include") || "",
      exclude: root.getAttribute("data-ttwc-exclude") || ""
    };
    var baseUrl = ajaxUrl || restUrl;
    var url;

    if (!baseUrl || root.getAttribute("data-ttwc-dynamic") !== "1") {
      return "";
    }
    ...
    url.searchParams.set("_ttwc_nocache", Date.now().toString(36) + Math.random().toString(36).slice(2));
  ...
  function refreshDynamicItems(root) {
    ...
    if (!track || !refreshUrl || root.__ttwcRefreshStarted) {
      return Promise.resolve();
    }

    root.__ttwcRefreshStarted = true;
    ...
    return fetch(refreshUrl, {
      ...
      .then(function (response) {
        if (!response.ok) {
          throw new Error("TT job working cards refresh failed");
        }
  ...
  function initSlider(root) {
    if (root.__ttwcInitialized) {
      return;
    }

    root.__ttwcInitialized = true;
    setupSlider(root);
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (sliders.length) {
      document.documentElement.classList.add("ttwc-slider-page");
      document.body.classList.add("ttwc-slider-page");
    }
