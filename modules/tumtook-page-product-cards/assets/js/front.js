(function () {
  ...
  function setupSlider(root) {
    ...
    function getReachableIndexes() {
      var positions = [];
      var groupSize = isMobileGroupMode() ? config.mobileGroupSize : 1;

      slides.forEach(function (slide, index) {
        var left;
        var isDuplicate;

        if (groupSize > 1 && index % groupSize !== 0 && index !== slides.length - 1) {
          return;
        }

        left = getSlideScrollLeft(slide);
        isDuplicate = positions.some(function (position) {
    ...
    function dragViewport(event) {
      ...
      var elapsed = Math.max(currentTime - dragLastTime, 8);
      var instantVelocity = (dragLastX - event.clientX) / elapsed;
    ...
    function stopViewportDrag() {
      ...
      if (shouldUpdateActiveCard) {
        if (!startMomentumScroll(dragVelocity, function () {
