...
final class Tumtook_Page_Product_Cards
{
	...
	public function register_meta_box()
	{
		add_meta_box(
			...
			__('Card Products  ทั้งหมด', 'tumtook-page-product-cards'),
	...
	public function render_meta_box($post)
	{
		...
		<style>
			.ttpc-admin-wrap,
			.ttpc-admin-wrap button,
			.ttpc-admin-wrap input,
			.ttpc-admin-wrap select,
			.ttpc-admin-wrap textarea {
			...
			.ttpc-admin-field--full {
				grid-column: 1/-1
			}

			.ttpc-admin-field label {
				font-weight: 600
			}

			.ttpc-admin-field input,
			.ttpc-admin-field select,
			.ttpc-admin-field textarea {
				width: 100%;
				max-width: none;
				box-sizing: border-box
			}

			.ttpc-admin-field input[type="number"] {
				-moz-appearance: textfield
			}

			.ttpc-admin-checklist {
				display: grid;
				gap: 8px
			}
		...
		<div class="ttpc-admin-wrap">
			<div class="ttpc-admin-panel">
				...
				<p class="ttpc-admin-note">
					<?php esc_html_e('ข้อมูลส่วนนี้จะถูกใช้เมื่อ page นี้ถูกเลือกไปแสดงใน section สินค้าเพิ่มเติมของหน้าอื่น', 'tumtook-page-product-cards'); ?>
				</p>
				<div class="ttpc-admin-grid" style="margin-top:16px">
					<div class="ttpc-admin-field ttpc-admin-field--full">
						<label
							for="ttpc-page-card-title"><?php esc_html_e('ชื่อที่แสดงบนการ์ด', 'tumtook-page-product-cards'); ?></label>
						<input id="ttpc-page-card-title" type="text" name="ttpc_page_meta[title]"
							value="<?php echo esc_attr($page_meta['title']); ?>"
							placeholder="<?php echo esc_attr(get_the_title($post)); ?>" />
						<p class="ttpc-admin-hint">
							<?php esc_html_e('ถ้าเว้นว่างไว้ ปลั๊กอินจะใช้ Title ของ page นี้แทน', 'tumtook-page-product-cards'); ?>
						</p>
					</div>
					<div class="ttpc-admin-field ttpc-admin-field--full">
						<label for="ttpc-page-price"><?php esc_html_e('ราคา', 'tumtook-page-product-cards'); ?></label>
						<input id="ttpc-page-price" type="text" name="ttpc_page_meta[price]"
							value="<?php echo esc_attr($page_meta['price']); ?>" placeholder="฿1.70" />
						<p class="ttpc-admin-hint">
							<?php esc_html_e('ถ้าใส่เฉพาะตัวเลข ระบบจะเติมสัญลักษณ์ ฿ ให้อัตโนมัติ', 'tumtook-page-product-cards'); ?>
						</p>
					</div>
					<div class="ttpc-admin-field ttpc-admin-field--full">
						<label><?php esc_html_e('รูปการ์ดของ page นี้', 'tumtook-page-product-cards'); ?></label>
						<input type="hidden" id="ttpc-page-image-id" name="ttpc_page_meta[image_id]"
							value="<?php echo esc_attr($page_meta['image_id']); ?>" />
						<div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
							<button type="button" class="button button-secondary ttpc-admin-button"
								id="ttpc-page-image-select"><?php esc_html_e('เลือกภาพ', 'tumtook-page-product-cards'); ?></button>
							<button type="button" class="button button-link-delete ttpc-admin-button"
								id="ttpc-page-image-clear"><?php esc_html_e('ลบภาพ', 'tumtook-page-product-cards'); ?></button>
						</div>
						...
						<p class="ttpc-admin-hint">
							<?php esc_html_e('ถ้าไม่ได้เลือกรูป ปลั๊กอินจะใช้ Featured Image ของ page นี้แทน', 'tumtook-page-product-cards'); ?>
						</p>
					</div>
				</div>
			</div>
			...
			<div class="ttpc-admin-panel">
				...
				<p class="ttpc-admin-note">
					<?php esc_html_e('เลือกว่าจะให้หน้านี้แสดงสินค้าเพิ่มเติมจาก page ไหนบ้าง โดยใส่ ID ของ page ที่ต้องการแสดง', 'tumtook-page-product-cards'); ?>
				</p>
				...
				<div class="ttpc-admin-grid" style="margin-top:16px">
					<div class="ttpc-admin-field">
						...
						<input id="ttpc-title" type="text" name="ttpc_settings[title]"
							value="<?php echo esc_attr($settings['title']); ?>" />
					</div>
					<div class="ttpc-admin-field">
						<label for="ttpc-limit"><?php esc_html_e('จำนวนการ์ด', 'tumtook-page-product-cards'); ?></label>
						<input id="ttpc-limit" type="number" min="0" max="99" name="ttpc_settings[limit]"
							value="<?php echo esc_attr($settings['limit']); ?>" />
						<p class="ttpc-admin-hint">
							<?php esc_html_e('ใส่ 0 เพื่อดึงทุกการ์ดที่เลือกไว้ทั้งหมด หากไม่ต้องการให้แสดง section นี้ ให้เอาติ๊ก "เปิดใช้งาน section นี้" ออกแทน', 'tumtook-page-product-cards'); ?>
						</p>
					</div>
					<div class="ttpc-admin-field">
						<label
							for="ttpc-view-all-label"><?php esc_html_e('ข้อความลิงก์ทั้งหมด', 'tumtook-page-product-cards'); ?></label>
						<input id="ttpc-view-all-label" type="text" name="ttpc_settings[view_all_label]"
							value="<?php echo esc_attr($settings['view_all_label']); ?>" />
					</div>
					<div class="ttpc-admin-field">
						<label
							for="ttpc-view-all-url"><?php esc_html_e('ลิงก์ทั้งหมด', 'tumtook-page-product-cards'); ?></label>
						<input id="ttpc-view-all-url" type="url" name="ttpc_settings[view_all_url]"
					...
					<div class="ttpc-admin-field">
						...
						<input id="ttpc-button-label" type="text" name="ttpc_settings[button_label]"
							value="<?php echo esc_attr($settings['button_label']); ?>" />
					</div>
					<div class="ttpc-admin-field">
						<label
							for="ttpc-section-state"><?php esc_html_e('การเปิดใช้งาน', 'tumtook-page-product-cards'); ?></label>
						<input id="ttpc-section-state" type="text"
							value="<?php echo '1' === $settings['enabled'] ? esc_attr__('เปิดใช้งานอยู่', 'tumtook-page-product-cards') : esc_attr__('ปิดการใช้งานอยู่', 'tumtook-page-product-cards'); ?>"
							readonly />
						<p class="ttpc-admin-hint">
							<?php esc_html_e('หากไม่ต้องการให้แสดง section นี้ ให้เอาติ๊ก "เปิดใช้งาน section นี้" ออกได้เลย', 'tumtook-page-product-cards'); ?>
						</p>
					</div>
					<div class="ttpc-admin-field ttpc-admin-field--full">
						<label
							for="ttpc-related-page-picker"><?php esc_html_e('Page ที่ต้องการแสดง', 'tumtook-page-product-cards'); ?></label>
						<input type="hidden" id="ttpc-related-page-ids" name="ttpc_settings[related_page_ids]"
							value="<?php echo esc_attr($settings['related_page_ids']); ?>" />
						<select id="ttpc-related-page-picker">
							<option value=""><?php esc_html_e('เลือก page ที่ต้องการเพิ่ม', 'tumtook-page-product-cards'); ?>
							</option>
						...
						<div class="ttpc-selected-pages" id="ttpc-selected-pages">
							<?php foreach ($selected_ids as $selected_id): ?>
								...
								<span class="ttpc-page-chip" data-page-id="<?php echo esc_attr($selected_id); ?>">
									...
									<button type="button" class="ttpc-page-chip__remove"
										aria-label="<?php esc_attr_e('Remove page', 'tumtook-page-product-cards'); ?>">×</button>
								</span>
							<?php endforeach; ?>
						</div>
						<p class="ttpc-admin-hint">
				...
				<p class="ttpc-admin-note">
					<?php esc_html_e('Shortcode ที่ใช้ได้: [tumtook_product_cards]', 'tumtook-page-product-cards'); ?>
				</p>
			</div>
		</div>
		...
		<section class="ttpc-section" data-ttpc-slider id="<?php echo esc_attr($instance_id); ?>">
			<div class="ttpc-shell">
				...
				<?php if ($using_placeholders && $this->is_editor_preview_context()): ?>
					<div class="ttpc-preview-note">
						<?php esc_html_e('ตอนนี้ยังไม่มี page ที่ถูกเลือกไว้ จึงแสดงตัวอย่าง layout ให้ก่อน', 'tumtook-page-product-cards'); ?>
					</div>
				<?php endif; ?>
								...
								<div class="ttpc-content">
									...
									<div class="ttpc-footer">
										<a class="ttpc-button" href="<?php echo esc_url($item['url']); ?>">
											...
											<span
												class="ttpc-button-label"><?php echo esc_html($settings['button_label']); ?></span>
										</a>
									</div>
								</div>
							</article>
				...
				<?php if ($show_controls): ?>
					<div class="ttpc-controls">
						...
						<div class="ttpc-arrows">
							<button type="button" class="ttpc-arrow ttpc-arrow--prev" data-ttpc-prev
								aria-label="<?php esc_attr_e('Previous cards', 'tumtook-page-product-cards'); ?>">
								<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
									<path d="M14.5 6.5L9 12l5.5 5.5" />
								</svg>
							</button>
							<button type="button" class="ttpc-arrow ttpc-arrow--next" data-ttpc-next
								aria-label="<?php esc_attr_e('Next cards', 'tumtook-page-product-cards'); ?>">
								<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
