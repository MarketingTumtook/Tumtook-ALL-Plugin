(function () {
  var config = {
    root: "[data-tthpr-slider]",
    track: "[data-tthpr-track]",
    slide: ".tthpr-card",
    dotClass: "tthpr-dot",
    dots: "[data-tthpr-pagination]",
    prev: "[data-tthpr-prev]",
    next: "[data-tthpr-next]",
    cssPrefix: "tthpr",
    pageLabel: "ไปยังกลุ่มสินค้า "
  };
  ...
  function setupSlider(root) {
    ...
    track.addEventListener("click", function (event) {
      ...
      card = event.target.closest(".tthpr-card[data-card-url]");
  ...
  function initSliders(scope) {
    ...
    var sliders = Array.prototype.slice.call(container.querySelectorAll(config.root));

    if (container.matches && container.matches(config.root)) {
      sliders.unshift(container);
    }

    if (sliders.length) {
      document.documentElement.classList.add("tthpr-slider-page");
      document.body.classList.add("tthpr-slider-page");
    }

    sliders.forEach(function (slider) {
      if (slider.__tthprInitialized) {
        return;
      }
      slider.__tthprInitialized = true;
      setupSlider(slider);
    });
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () { initSliders(document); });
  } else {
    initSliders(document);
  }

  var elementorHookAdded = false;
  function bindElementor() {
    if (elementorHookAdded || !window.elementorFrontend || !window.elementorFrontend.hooks) {
      return;
    }
    elementorHookAdded = true;
    window.elementorFrontend.hooks.addAction("frontend/element_ready/global", function (scope) {
      initSliders(scope && scope[0] ? scope[0] : scope);
    });
  }

  bindElementor();
  if (window.jQuery) {
    window.jQuery(window).on("elementor/frontend/init.tthpr", bindElementor);
  }
})();
