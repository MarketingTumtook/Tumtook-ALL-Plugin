...
final class Tumtook_Brand_Showcase_Plugin {
	...
	private function get_default_data() {
		return array(
			'title'          => __( 'Brand Showcase', 'tumtook-brand-showcase' ),
		...
		<div class="ttbs-admin-field ttbs-admin-field--full">
			...
			<div class="ttbs-admin-media">
				<div class="ttbs-admin-media__inputs">
					<input type="hidden" class="ttbs-media-id" name="<?php echo esc_attr( $args['id_name'] ); ?>" value="<?php echo esc_attr( $args['id_value'] ); ?>" />
					<input type="url" class="ttbs-media-url" name="<?php echo esc_attr( $args['url_name'] ); ?>" value="<?php echo esc_url( $args['url_value'] ); ?>" placeholder="<?php esc_attr_e( 'Upload or paste image URL', 'tumtook-brand-showcase' ); ?>" />
					<input type="text" name="<?php echo esc_attr( $args['alt_name'] ); ?>" value="<?php echo esc_attr( $args['alt_value'] ); ?>" placeholder="<?php esc_attr_e( 'Image alt text (optional)', 'tumtook-brand-showcase' ); ?>" />
					<input type="url" name="<?php echo esc_attr( $args['link_name'] ); ?>" value="<?php echo esc_url( $args['link_value'] ); ?>" placeholder="<?php esc_attr_e( 'Card link URL (optional)', 'tumtook-brand-showcase' ); ?>" />
				</div>
				<div class="ttbs-admin-media__actions">
					<button type="button" class="button button-secondary ttbs-upload"><?php esc_html_e( 'Select Image', 'tumtook-brand-showcase' ); ?></button>
					<button type="button" class="button button-link-delete ttbs-clear"><?php esc_html_e( 'Clear', 'tumtook-brand-showcase' ); ?></button>
				</div>
			</div>
	...
	public function render_page_meta_box( $post ) {
		...
		?>
		<style>
			.ttbs-admin-wrap{display:grid;gap:18px}
			.ttbs-admin-intro{margin:0;color:#50575e}
			.ttbs-admin-section{background:#fff;border:1px solid #dcdcde;border-radius:18px;padding:18px}
			.ttbs-admin-title{margin:0 0 14px;font-size:15px;font-weight:700}
			.ttbs-admin-grid{display:grid;gap:16px;grid-template-columns:repeat(2,minmax(0,1fr))}
			.ttbs-admin-header-fields{display:grid;gap:12px;margin-bottom:18px}
			.ttbs-admin-slides{display:grid;gap:16px;grid-template-columns:repeat(2,minmax(0,1fr));align-items:start}
			.ttbs-admin-slide-card{border:1px solid #e2e4e7;border-radius:16px;padding:14px;background:#fcfcfd}
			.ttbs-admin-slide-card h4{margin:0 0 10px;font-size:14px;font-weight:700}
			.ttbs-admin-field label{display:block;font-weight:600;margin-bottom:6px}
			.ttbs-admin-field input,.ttbs-admin-field textarea{width:100%}
			.ttbs-admin-field--full{grid-column:1/-1}
			.ttbs-admin-media{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:start}
			.ttbs-admin-media__inputs{display:grid;gap:10px}
			.ttbs-admin-media__actions{display:grid;gap:8px;justify-items:start}
			.ttbs-admin-preview{margin-top:10px}
			.ttbs-admin-preview img{width:100%;max-width:100%;height:140px;object-fit:cover;border-radius:14px;display:block;box-shadow:0 8px 22px rgba(0,0,0,.08)}
			@media (max-width: 782px){
				.ttbs-admin-grid,.ttbs-admin-slides{grid-template-columns:1fr}
				.ttbs-admin-field--full{grid-column:auto}
				.ttbs-admin-media{grid-template-columns:1fr}
			}
		</style>
			...
			<section class="ttbs-admin-section">
				...
				<div class="ttbs-admin-header-fields">
					<div class="ttbs-admin-field">
						...
						<input id="ttbs-title" type="text" name="tumtook_brand_showcase_data[title]" value="<?php echo esc_attr( $data['title'] ); ?>" />
					</div>
					<div class="ttbs-admin-grid">
						<div class="ttbs-admin-field">
							<label for="ttbs-view-all-label"><?php esc_html_e( 'Right link label', 'tumtook-brand-showcase' ); ?></label>
							<input id="ttbs-view-all-label" type="text" name="tumtook_brand_showcase_data[view_all_label]" value="<?php echo esc_attr( $data['view_all_label'] ); ?>" />
						</div>
						<div class="ttbs-admin-field">
							<label for="ttbs-view-all-url"><?php esc_html_e( 'Right link URL', 'tumtook-brand-showcase' ); ?></label>
							<input id="ttbs-view-all-url" type="url" name="tumtook_brand_showcase_data[view_all_url]" value="<?php echo esc_url( $data['view_all_url'] ); ?>" />
						</div>
					</div>
				</div>
	...
	private function build_slider_markup( $page_id, $slides, $args ) {
		...
		<section class="ttbs-showcase" id="<?php echo esc_attr( $instance_id ); ?>" data-autoplay-delay="4500">
				...
				<?php foreach ( $slides as $index => $slide ) : ?>
					<article class="ttbs-showcase__slide<?php echo 0 === $index ? ' is-active' : ''; ?>" data-slide>
						<div class="ttbs-showcase__card">
							<img class="ttbs-showcase__image" src="<?php echo esc_url( $slide['image_url'] ); ?>" alt="<?php echo esc_attr( $slide['alt'] ); ?>" loading="lazy" />
							<?php if ( ! empty( $slide['link_url'] ) ) : ?>
								<a class="ttbs-showcase__card-link" href="<?php echo esc_url( $slide['link_url'] ); ?>" aria-label="<?php esc_attr_e( 'Open brand link', 'tumtook-brand-showcase' ); ?>">
									<span class="ttbs-showcase__card-link-icon" aria-hidden="true"></span>
								</a>
							<?php endif; ?>
						</div>
					</article>
			...
			<div class="ttbs-showcase__nav" aria-label="<?php esc_attr_e( 'Brand showcase controls', 'tumtook-brand-showcase' ); ?>">
				<div class="ttbs-showcase__dots" data-slider-dots></div>
				<div class="ttbs-showcase__buttons">
					<button type="button" class="ttbs-showcase__button ttbs-showcase__button--prev" data-slider-prev aria-label="<?php esc_attr_e( 'Previous slide', 'tumtook-brand-showcase' ); ?>">
						<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
							<path d="M14.5 6.5L9 12l5.5 5.5" />
						</svg>
					</button>
					<button type="button" class="ttbs-showcase__button ttbs-showcase__button--next" data-slider-next aria-label="<?php esc_attr_e( 'Next slide', 'tumtook-brand-showcase' ); ?>">
						<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
