...
final class API_Catalog_Images_Plugin
{
	...
	private function get_values_by_path($source, $path)
	{
		...
		$this->collect_path_values($source, $segments, 0, $results);

		return $results;
	}

	private function collect_path_values($current, $segments, $index, &$results)
	{
		...
		if (is_array($current) && array_key_exists($segment, $current)) {
			$this->collect_path_values($current[$segment], $segments, $index + 1, $results);
		}
	}
